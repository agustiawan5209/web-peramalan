...
export default function TrainModels({ indikator, transactionX, transactionY, onModelsTrained }: TrainModelsProps) {
  ...
  const trainAllModels = async () => {
    ...
    try {
      ...
      const InputXs = transactionX.map(point =>
        indikator.map((_, i) => normalize(point[i], featureRanges[i].min, featureRanges[i].max))
      );
      ...
      const trainSingleModel = async (
        model: tf.Sequential,
        outputKey: keyof typeof transactionY[0],
        ...
        const ys = tf.tensor2d(
          transactionY.map(point => [
            normalize(point[outputKey], outputMin, outputMax)
          ])
        );
      ...
      const [
        conttoniBasahParams,
        conttoniKeringParams,
        spinosumBasahParams,
        spinosumKeringParams
      ] = await Promise.all([
      ...
      <div className="mb-4 space-y-2">
        ...
        <div className="h-2 w-full rounded-full bg-gray-200">
          <div
            ...
            style={{ width: `${progress.conttoniBasah}%` }}
          ></div>
        </div>
      </div>
